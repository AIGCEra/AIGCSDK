...
namespace CommonUniverse {
	...
	BOOL CWebRTApp::InitApplication() {
		if (WebRTInit(_T(""))) {
			ProcessAppType(m_bCrashReporting);
			return true;
		}
		else
			return false;
	...
	bool CWebRTApp::ProcessAppType(bool bCrashReporting) {
		BOOL bWin32 =
			(::PathFileExists(m_pWebRTImpl->m_strAppPath + m_strExeName + _T(".app.html")) == false) &&
			(::PathFileExists(m_pWebRTImpl->m_strAppPath + m_strExeName + _T(".html")) == false);
		if (bWin32)
			return true;
		if (m_pWebRTImpl) {
