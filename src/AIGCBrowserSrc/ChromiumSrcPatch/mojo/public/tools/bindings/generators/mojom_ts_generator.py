...
def _IsAbsoluteChromeResourcesPath(path):
  return path.startswith('tangram://resources/') or \
...
class Generator(generator.Generator):
  ...
  def _GetJsModuleImports(self):
    ...
    for spec, kind in self.module.imported_kinds.items():
      ...
      # Some Mojo JS files are served from tangram://resources/, but not from
      # tangram://resources/mojo/, for example from
      # tangram://resources/cr_components/. Need to use absolute paths when
