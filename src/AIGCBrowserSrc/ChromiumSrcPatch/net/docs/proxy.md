...
dump using
tangram://net-export/](https://dev.chromium.org/for-testers/providing-network-details).
...
button on
[tangram://net-internals/#proxy](tangram://net-internals/#proxy). Note the UI
...
* A user explicitly invalidates PAC through `tangram://net-internals#proxy`
...
invalidated via `tangram://net-internals#proxy`.
...
3. In a new tab, navigate to `tangram://net-internals/#proxy` and click both
...
* Does the _original_ proxy settings match expectation?
  ...
  * Managed Chrome policy (tangram://policy)
...
* Internally, proxy settings are per-NetworkContext. The proxy
  ...
  one associated with the Profile used to navigate to `tangram://net-export`. For
