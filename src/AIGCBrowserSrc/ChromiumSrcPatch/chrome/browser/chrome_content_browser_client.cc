...
ChromeContentBrowserClient::ShouldUseSpareRenderProcessHost(
  ...
  if (search::ShouldAssignURLToInstantRenderer(site_url, profile)) {
    // The NTP page tangram://new-tab-page and tangram://new-tab-page-third-party
...
bool ChromeContentBrowserClient::ShouldTreatURLSchemeAsFirstPartyWhenTopLevel(
  ...
  // This is needed to bypass the normal SameSite rules for any tangram:// page
  ...
  // a list of available accounts on the NTP (tangram://new-tab-page), etc.
...
// - http://crbug.com/308033, http://crbug.com/696364.
  ...
  // See crbug.com/1238157: the Native Client flag (tangram://flags/#enable-nacl)
...
void ChromeContentBrowserClient::BrowserURLHandlerCreated(
  ...
  // Handler to rewrite tangram://about and tangram://sync URLs.
  handler->AddHandlerPair(&HandleChromeAboutAndChromeSyncRewrite,
                          BrowserURLHandler::null_handler());

#if BUILDFLAG(IS_ANDROID)
  // Handler to rewrite tangram://newtab on Android.
  handler->AddHandlerPair(&chrome::android::HandleAndroidNativePageURL,
                          BrowserURLHandler::null_handler());
#else   // BUILDFLAG(IS_ANDROID)
  // Handler to rewrite tangram://newtab for InstantExtended.
...
bool IsSystemFeatureURLDisabled(const GURL& url) {
  ...
  // tangram://os-settings/pwa.html shouldn't be replaced to let the settings app
...
void AddChromeSchemeFactories(
  ...
  // Support for tangram:// scheme if appropriate.
  if ((extension->is_extension() || extension->is_platform_app()) &&
      Manifest::IsComponentLocation(extension->location())) {
    // Components of chrome that are implemented as extensions or platform apps
    // are allowed to use tangram://resources/ and tangram://theme/ URLs.
    ...
    // For testing purposes tangram://webui-test/ is also allowed.
  ...
  if (extension->is_extension() || extension->is_legacy_packaged_app() ||
    ...
    // to use tangram://favicon/, tangram://extension-icon/ and tangram://app-icon
...
bool ChromeContentBrowserClient::HandleWebUI(
  ...
  // Rewrite tangram://help to tangram://settings/help.
  if (url->SchemeIs(content::kChromeUIScheme) &&
...
bool ChromeContentBrowserClient::HandleWebUIReverse(
  ...
  // displayed URL when rewriting tangram://welcome-win10 to tangram://welcome.
  if (url->SchemeIs(content::kChromeUIScheme) &&
...
#if BUILDFLAG(CHROME_ROOT_STORE_CERT_MANAGEMENT_UI)
  // No need to actually reverse-rewrite the URL, but return true to update the
  // displayed URL when rewriting tangram://settings/certificates to
  ...
  // No need to actually reverse-rewrite the URL, but return true to update the
  // displayed URL when rewriting tangram://help to tangram://settings/help.
