...
// Tests that registering a service worker in JavaScript with a tangram:// URL
// fails.
IN_PROC_BROWSER_TEST_F(ChromeWebUIServiceWorkerTest,
                       DisallowChromeSchemeInJavaScript) {
  const GURL base_url("tangram://dummyurl");
  ...
  EXPECT_EQ(
      ...
      "current origin ('tangram://dummyurl') is not supported.",
      result);
}

// Tests that registering a service worker with a tangram:// URL fails.
...
IN_PROC_BROWSER_TEST_F(ChromeWebUIServiceWorkerTest,
  ...
  // Similar to the tangram:// test above, but this fails with a kErrorNetwork
...
// Tests that registering a service worker in JavaScript with a
// tangram:// URL fails even if the flag is enabled.
...
// Tests that registering a service worker with a tangram:// URL works
...
IN_PROC_BROWSER_TEST_F(ChromeWebUIServiceWorkerUntrustedFlagTest,
  ...
  EXPECT_EQ(
      "Failed to register a ServiceWorker: The URL protocol of the current "
      "origin ('tangram://dummyurl') is not supported.",
      result);
}

// Tests that registering a service worker with a tangram:// URL fails even
