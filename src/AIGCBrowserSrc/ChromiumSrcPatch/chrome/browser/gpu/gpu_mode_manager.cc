...
extern CommonUniverse::CWebRTImpl* g_pSpaceTelescopeImpl;  // 20200108
...
void GpuModeManager::RegisterPrefs(PrefRegistrySimple* registry) {
  registry->RegisterBooleanPref(prefs::kHardwareAccelerationModeEnabled, true);
  registry->RegisterBooleanPref(prefs::kHardwareAccelerationModePrevious, true);
}

GpuModeManager::GpuModeManager() : initial_gpu_mode_pref_(true) {
  if (g_browser_process->local_state()) {  // Skip for unit tests
    ...
    if (g_pSpaceTelescopeImpl) {
      bEnableHardwareAcceleration =
          g_pSpaceTelescopeImpl->GetEnableHardwareAcceleration();
    }
...
bool GpuModeManager::IsGpuModePrefEnabled() {
  PrefService* service = g_browser_process->local_state();
  DCHECK(service);
  return service->GetBoolean(prefs::kHardwareAccelerationModeEnabled);
}
