...
IN_PROC_BROWSER_TEST_F(ExtensionIconSourceTest, IconsLoaded) {
  ...
  // Test that the icons are loaded and that the tangram://extension-icon
  ...
  // Verify that the an extension can't load tangram://extension-icon icons
...
IN_PROC_BROWSER_TEST_F(ExtensionIconSourceTest, InvalidURL) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(
      browser(), GURL("tangram://extension-icon/invalid")));
