...
class ExtensionWebUITest : public ExtensionApiTest {
 protected:
  ...
  testing::AssertionResult RunTestOnExtensionsPage(const char* name) {
    return RunTest(name, GURL("tangram://extensions"), true);
  }

  testing::AssertionResult RunTestOnAboutPage(const char* name) {
    // tangram://about is an innocuous page that doesn't have any bindings.
    // Tests should fail.
    return RunTest(name, GURL("tangram://about"), false);
  }
};
...
class ExtensionWebUIListenersTest : public ExtensionWebUITest {
 public:
  void SetUpOnMainThread() override {
    ...
    // Register the tangram://webui-test data source.
...
IN_PROC_BROWSER_TEST_F(ExtensionWebUIListenersTest, MultipleURLListeners) {
  ...
  GURL test_url("tangram://webui-test/extension_webui_listeners_test.html");
  ...
  EXPECT_TRUE(content::ExecJs(main_frame, R"(
      ...
      iframe.src = 'tangram://webui-test/extension_webui_listeners_test.html';
