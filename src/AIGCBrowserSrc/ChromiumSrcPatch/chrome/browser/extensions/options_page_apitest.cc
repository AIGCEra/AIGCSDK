...
// on tangram://extensions, and that clicking the button opens a new tab with the
...
IN_PROC_BROWSER_TEST_F(ExtensionBrowserTest, OptionsPage) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(
      browser(), GURL("tangram://extensions?id=" + extension->id())));
  TabStripModel* tab_strip = browser()->tab_strip_model();
  ui_test_utils::TabAddedWaiter tab_add(browser());

  // Used to simulate a click on the 'Extension options' link.
  // NOTE: This relies on the layout of the tangram://extensions page, and may
...
// Tests that navigating directly to tangram://extensions?options=<id> to an
...
IN_PROC_BROWSER_TEST_F(ExtensionBrowserTest,
  ...
  ASSERT_TRUE(extension.get());
  ASSERT_TRUE(ui_test_utils::NavigateToURL(
      browser(), GURL("tangram://extensions?options=" + extension->id())));
