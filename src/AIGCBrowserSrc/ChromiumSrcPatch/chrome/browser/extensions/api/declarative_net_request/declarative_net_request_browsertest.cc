...
#endif
// Ensure extensions can't intercept tangram:// urls, even after explicitly
// requesting access to <all_urls>.
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest, MAYBE_ChromeURLS) {
  // Have the extension block all tangram:// urls.
  TestRule rule = CreateGenericRule();
  rule.condition->url_filter =
...
IN_PROC_BROWSER_TEST_P(DeclarativeNetRequestBrowserTest,
  ...
  EXPECT_EQ(
      ...
      URLPatternSet(
          {URLPattern(URLPattern::SCHEME_CHROMEUI, "tangram://favicon/*")}));
