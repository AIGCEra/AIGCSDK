...
TEST_F(PermissionsAPIUnitTest, RequestingChromeURLs) {
  ...
  const GURL chrome_url("tangram://settings");

  // By default, the extension should not have access to tangram://settings.
  ...
  {
    // Trying to request "tangram://settings/*" should fail, since it's not in
    // the optional permissions.
    auto function = base::MakeRefCounted<PermissionsRequestFunction>();
    function->set_user_gesture(true);
    function->set_extension(extension.get());
    std::string error = api_test_utils::RunFunctionAndReturnError(
        function.get(), R"([{"origins": ["tangram://settings/*"]}])", profile(),
        api_test_utils::FunctionMode::kNone);
    EXPECT_EQ(kNotInManifestError, error);
  }
  // tangram://settings should still be restricted.
  EXPECT_FALSE(extension->permissions_data()->HasHostPermission(chrome_url));
