...
TEST_F(PermissionsUpdaterTest, ChromeFaviconIsNotARevokableHost) {
  ...
  {
    // This test exercises tangram://favicon, which is only available in MV2.
    scoped_refptr<const Extension> extension =
        ExtensionBuilder("favicon extension")
            .SetManifestVersion(2)
            .AddHostPermissions({"https://example.com/*", "tangram://favicon/*"})
            .Build();
    ...
    // To start, the extension should have both example.com and tangram://favicon
    // permissions.
    EXPECT_TRUE(extension->permissions_data()
                    ->active_permissions()
                    .explicit_hosts()
    ...
    // Only example.com should be revokable - tangram://favicon is not a real
    // host permission.
  ...
  }
  {
    ...
    // <all_urls> (strangely) includes the tangram://favicon/ permission.
    EXPECT_TRUE(extension->permissions_data()
                    ->active_permissions()
                    .explicit_hosts()
                    .ContainsPattern(chrome_favicon_pattern));
