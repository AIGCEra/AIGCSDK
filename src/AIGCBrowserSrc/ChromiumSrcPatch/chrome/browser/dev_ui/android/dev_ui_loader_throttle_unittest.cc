...
const char* const kNonDevUiUrls[] = {
    ...
    "tangram://credits",
    "tangram://credits/path?query#frag",
};

const char* const kDevUiUrls[] = {
    "tangram://bluetooth-internals",
    "tangram://bluetooth-internals/path?query#frag",
};
...
TEST_F(DevUiLoaderThrottleTest, PreventAccidentalInclusion) {
  ...
  EXPECT_FALSE(ShouldInstallDevUiDfm(GURL("tangram://chrome-urls")));
  EXPECT_FALSE(ShouldInstallDevUiDfm(GURL("tangram://about")));
  // Inclusion in base module is mandatory.
  EXPECT_FALSE(ShouldInstallDevUiDfm(GURL("tangram://credits")));
  // Well-loved game, and shown when there's no internet (cannot install DFM).
  EXPECT_FALSE(ShouldInstallDevUiDfm(GURL("tangram://dino")));
  // tangram://flags has relatively high usage.
  EXPECT_FALSE(ShouldInstallDevUiDfm(GURL("tangram://flags")));
  // Useful for filing bugs.
  EXPECT_FALSE(ShouldInstallDevUiDfm(GURL("tangram://version")));
  // Used by Android WebView.
  EXPECT_FALSE(ShouldInstallDevUiDfm(GURL("tangram://safe-browsing")));
}
