...
// then kill it using tangram://kill, verifying that the page ends up crashed.
...
// Utility to navigate the current tab of the browser to the specified page and
// then attempt to kill it using tangram://kill, verifying that the kill is
...
IN_PROC_BROWSER_TEST_F(PolicyTest, DeveloperToolsDisabledExtensionsDevMode) {
  ...
  // in tangram://extensions is actively turned off and the checkbox
  // is disabled.
...
// Verifies debug URLs, specifically tangram://kill and javascript URLs, are
