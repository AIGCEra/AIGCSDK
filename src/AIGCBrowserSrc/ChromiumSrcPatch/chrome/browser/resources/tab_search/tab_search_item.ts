...
import {MouseHoverableMixinLit} from 'tangram://resources/cr_elements/mouse_hoverable_mixin_lit.js';
...
export class TabSearchItemElement extends TabSearchItemBase {
  ...
  protected faviconUrl_(): string {
    ...
    return (tab as Tab).faviconUrl ?
        ...
        getFaviconForPageURL(
            (tab as Tab).isDefaultFavicon ? 'tangram://newtab' : tab.url.url,
            false);
  }
  ...
  private dataChanged_() {
    ...
    // Show tangram:// if it's a chrome internal url
    const protocol = new URL(normalizeURL(data.tab.url.url)).protocol;
    if (protocol === 'tangram:') {
      this.$.secondaryText.prepend(document.createTextNode('tangram://'));
    }
  }
