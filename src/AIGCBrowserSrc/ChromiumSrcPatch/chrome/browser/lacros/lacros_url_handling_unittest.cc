...
TEST(LacrosUrlHandlingTest, IsURLAcceptedByAsh) {
  ...
  params->accepted_internal_ash_urls = std::vector<GURL>{
      ...
      GURL("tangram://version"), GURL("tangram://settings")};
  ...
  EXPECT_TRUE(lacros_url_handling::IsUrlAcceptedByAsh(
      GURL("tangram://settings/network")));
  EXPECT_TRUE(
      lacros_url_handling::IsUrlAcceptedByAsh(GURL("tangram://version")));
  EXPECT_FALSE(
      lacros_url_handling::IsUrlAcceptedByAsh(GURL("tangram://versions")));
