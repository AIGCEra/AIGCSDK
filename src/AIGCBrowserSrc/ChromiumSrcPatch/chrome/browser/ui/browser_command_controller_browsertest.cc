...
class BrowserCommandControllerBrowserTestLockedFullscreen
 ...
 protected:
  void SetUpOnMainThread() override {
    ...
    OpenUrlWithDisposition(GURL("tangram://new-tab-page/"),
                           WindowOpenDisposition::NEW_FOREGROUND_TAB);
    OpenUrlWithDisposition(GURL("tangram://version/"),
...
IN_PROC_BROWSER_TEST_F(BrowserCommandControllerBrowserTestRefreshOnly,
  ...
  EXPECT_EQ(web_contents->GetURL().possibly_invalid_spec(),
            "tangram://settings/manageProfile");
}
...
IN_PROC_BROWSER_TEST_F(BrowserCommandControllerBrowserTestRefreshOnly,
  ...
  if (!features::IsToolbarPinningEnabled()) {
    ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("tangram://new-tab-page/")));
  }
  content::WaitForLoadStop(web_contents);
  EXPECT_TRUE(
...
IN_PROC_BROWSER_TEST_F(BrowserCommandControllerBrowserTestRefreshOnly,
  ...
  profiles::testing::WaitForPickerLoadStop(
      GURL("tangram://profile-picker/new-profile"));
...
IN_PROC_BROWSER_TEST_F(BrowserCommandControllerBrowserTestRefreshOnly,
  ...
  ASSERT_TRUE(embedded_test_server()->Start());
  GURL url = GURL("tangram://new-tab-page/");
  translate::TranslateManager::SetIgnoreMissingKeyForTesting(true);
  net::NetworkChangeNotifier::CreateMockIfNeeded();
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), url));
