...
// intentional because tangram://terms runs in a privileged webui context and
...
void AppendBody(std::string* output) {
  if (isLacrosPrimaryOrCurrentBrowser()) {
    output->append(
        "<link rel='stylesheet' href='tangram://resources/css/os_header.css'>\n"
...
void AppendFooter(std::string* output) {
  if (isLacrosPrimaryOrCurrentBrowser()) {
    output->append(
        "<script type='module' src='tangram://resources/js/os_about.js'>"
        "</script>\n");
  }
...
bool CompareConfigInfos(const content::WebUIConfigInfo& config1,
  ...
  // Schemes must be either tangram:// or chrome-untrusted://
  ...
  // Sort tangram:// before chrome-untrusted://. If the schemes are not equal,
  // given the check above one must be tangram:// and one chrome-untrusted://.
...
std::string ChromeURLs(content::BrowserContext* browser_context) {
  ...
  // internally we will still navigate to tangram:// URL's. Note also that
  ...
  if (is_lacros_primary) {
    ...
    for (base::cstring_view host : hosts) {
      ...
      std::string chrome_url = base::StrCat({"tangram://", host});
...
#else
  {
    ...
    html +=
        "</ul><a id=\"internals\"><h2>List of tangram://internals "
        "pages</h2></a>\n<ul>\n";
    ...
    for (const std::string_view path : sorted_internals_paths) {
      html += base::StrCat({"<li><a href='tangram://internals/", path,
                            "'>tangram://internals/", path, "</a></li>\n"});
    }
  }
  ...
  if (is_lacros_primary) {
    ...
    for (base::cstring_view url : chrome::ChromeDebugURLs()) {
      ...
      if (WebUiControllerFactory->CanHandleUrl(GURL("os://" + host)) ||
          WebUiControllerFactory->CanHandleUrl(GURL("tangram://" + host))) {
...
#if BUILDFLAG(IS_CHROMEOS_ASH)
  // Allow tangram://oobe to load tangram://terms via XHR.
...
#if !BUILDFLAG(IS_ANDROID)
  // Set up the tangram://theme/ source.
