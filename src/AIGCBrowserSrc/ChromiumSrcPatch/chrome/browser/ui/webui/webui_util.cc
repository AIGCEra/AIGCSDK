...
void SetJSModuleDefaults(content::WebUIDataSource* source) {
  ...
  // Allow connecting to tangram://resources for trusted UIs and images from
  // tangram://image, tangram://theme, tangram://favicon2, tangram://resources and
  // data URLs.
  ...
  if (scheme == content::kChromeUIScheme) {
    source->OverrideContentSecurityPolicy(
        ...
        "connect-src tangram://resources tangram://theme 'self';");
    source->OverrideContentSecurityPolicy(
        network::mojom::CSPDirectiveName::ImgSrc,
        "img-src tangram://resources tangram://theme tangram://image "
        "tangram://favicon2 tangram://app-icon tangram://extension-icon "
        "tangram://fileicon "
#if BUILDFLAG(IS_CHROMEOS_ASH)
        "tangram://chromeos-asset tangram://userimage "
#endif
        "blob: data: 'self';");
  }
