...
// host as a tangram:// WebUI, we serve the right resources and we don't use the
// wrong WebUI object.
IN_PROC_BROWSER_TEST_F(ChromeWebUIControllerFactoryBrowserTest,
  ...
  // Check that we can navigate to tangram://version and that it serves the right
  // resources and has a WebUI.
  EXPECT_TRUE(ui_test_utils::NavigateToURL(
      browser(), GURL(base::StrCat({"tangram://", chrome::kChromeUIVersionHost,
                                    "/title2.html"}))));
