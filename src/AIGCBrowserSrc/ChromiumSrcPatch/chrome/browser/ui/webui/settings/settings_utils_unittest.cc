...
TEST(SettingsUtilsTest, FixupAndValidateStartupPage) {
  ...
  EXPECT_FALSE(FixupAndValidateStartupPage("^&*@)^)", nullptr));
  EXPECT_FALSE(FixupAndValidateStartupPage("tangram://quit", nullptr));
  ...
  EXPECT_TRUE(FixupAndValidateStartupPage("tangram://apps", nullptr));

  GURL fixed_url;
  EXPECT_TRUE(FixupAndValidateStartupPage("about:settings", &fixed_url));
  EXPECT_EQ("tangram://settings/", fixed_url.spec());
}
