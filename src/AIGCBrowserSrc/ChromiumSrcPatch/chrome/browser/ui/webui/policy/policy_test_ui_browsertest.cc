...
class PolicyTestPageVisibilityTest
 ...
 public:
  ...
  // Verifies that the policy test page at tangram://policy/test is visible if
...
// Verify that the tangram://policy/test page is visible only when both the flag
...
IN_PROC_BROWSER_TEST_P(PolicyTestPageVisibilityTest,
  ...
  if (!policy::utils::IsPolicyTestingEnabled(/*pref_service=*/nullptr,
                                             chrome::GetChannel())) {
    GTEST_SKIP() << "tangram://policy/test not allowed on this build.";
  }
...
IN_PROC_BROWSER_TEST_F(PolicyTestHandlerTest,
  ...
  // Ensure tangram://policy/test not supported.
...
IN_PROC_BROWSER_TEST_F(PolicyTestHandlerTest,
  ...
  // Open tangram://policy/test for the first time
  ...
  // Navigate away and re-open tangram://policy/test for the first time
  ASSERT_TRUE(content::NavigateToURL(web_contents(),
...
class PolicyTestUITest : public PlatformBrowserTest {
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    // Set profile to unmanaged so tangram://policy/test is accessible.
    policy::ScopedManagementServiceOverrideForTesting browser_management(
        policy::ManagementServiceFactory::GetForProfile(GetProfile()),
