...
IN_PROC_BROWSER_TEST_F(ExtensionSettingsUIBrowserTest, ViewSource) {
  ...
  GURL options_url("tangram://extensions/?options=" + extension->id());
...
// when there is a tangram://extensions page open. This is important, since some
...
IN_PROC_BROWSER_TEST_F(ExtensionSettingsUIBrowserTest, ListenerRegistration) {
  ...
  ui_test_utils::NavigateToURLWithDisposition(
      browser(), GURL("tangram://extensions"),
...
IN_PROC_BROWSER_TEST_F(ExtensionSettingsUIBrowserTest,
  ...
  // Navigate to tangram://extensions which is a allowlisted URL for the
  // chrome.activityLogPrivate API.
  GURL extensions_url("tangram://extensions");
...
IN_PROC_BROWSER_TEST_F(ExtensionsActivityLogTest, TestActivityLogVisible) {
  ...
  GURL activity_log_url("tangram://extensions/?activity=" + extension->id());
  ...
  // in the tangram://extensions page.
  ...
  EXPECT_EQ(true,
         ...
         const polymerPath =
             'tangram://resources/polymer/v3_0/polymer/polymer_bundled.min.js';
         Promise.all([
...
IN_PROC_BROWSER_TEST_F(SafetyHubExtensionSettingsUIBrowserTest,
  ...
  ASSERT_TRUE(
      ui_test_utils::NavigateToURL(browser(), GURL("tangram://extensions/")));
  notification = notification_service->GetNotificationToShow();
  ASSERT_FALSE(notification.has_value());
}
