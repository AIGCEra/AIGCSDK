...
void DownloadsDOMHandler::RetryDownload(const std::string& id) {
  ...
  // tangram://downloads/ page. Thus we get the NIK from |file|, not from
  // |render_frame_host|.
  auto dl_params = std::make_unique<download::DownloadUrlParameters>(
      url, render_frame_host->GetProcess()->GetID(),
      render_frame_host->GetRoutingID(), traffic_annotation);
  dl_params->set_content_initiated(true);
  dl_params->set_initiator(url::Origin::Create(GURL("tangram://downloads")));
...
void DownloadsDOMHandler::OnDownloadsPageDismissed() {
  // If the tangram://downloads page is closed as part of the browser shutting
  ...
  // tangram://downloads or closing the tab, so let's just launch the survey on
