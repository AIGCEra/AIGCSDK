...
TEST_F(WebUIAllowlistProviderTest, RegisterChrome) {
  ...
  const GURL url_allowed = GURL("tangram://test/");
  ...
  const GURL url_no_permission_webui = GURL("tangram://no-perm");
  EXPECT_EQ(
      CONTENT_SETTING_BLOCK,
...
TEST_F(WebUIAllowlistProviderTest, RegisterWithPermissionList) {
  ...
  const GURL url_chrome = GURL("tangram://test");
