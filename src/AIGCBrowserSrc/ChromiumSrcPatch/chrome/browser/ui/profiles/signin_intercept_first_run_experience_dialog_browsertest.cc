...
class SigninInterceptFirstRunExperienceDialogBrowserTest : public TestBase {
 ...
 protected:
  const GURL kSyncConfirmationUrl = AppendSyncConfirmationQueryParams(
      GURL("tangram://sync-confirmation"),
      SyncConfirmationStyle::kSigninInterceptModal,
      /*is_sync_promo=*/true);
  const GURL kProfileCustomizationUrl = GURL("tangram://profile-customization");
  const GURL kSyncSettingsUrl = GURL("tangram://settings/syncSetup");
