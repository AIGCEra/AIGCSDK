...
IN_PROC_BROWSER_TEST_F(BrowserViewTest, ShowFaviconInTab) {
  // Opens "tangram://version/" page, which uses default favicon.
...
IN_PROC_BROWSER_TEST_F(BrowserViewDataProtectionTest,
  ...
  // Initial page loaded into the browser view is a tangram:// URL that has no
  // watermark.
  EXPECT_FALSE(
...
IN_PROC_BROWSER_TEST_F(BrowserViewDataProtectionTest,
  ...
  ASSERT_TRUE(
      AddTabAtIndex(1, GURL("tangram://version"), ui::PAGE_TRANSITION_LINK));
  EXPECT_FALSE(BrowserView::GetBrowserViewForBrowser(browser())
                   ->get_watermark_view_for_testing()
                   ->has_text_for_testing());
...
IN_PROC_BROWSER_TEST_F(BrowserViewDataProtectionTest,
  ...
  // not be showing.
  NavigateParams params(browser(), GURL("tangram://version"),
