...
gfx::Rect BrowserFrameViewWin::GetBoundsForTabStripRegion(
  ...
  if (!CaptionButtonsOnLeadingEdge()) {
    ...
    if (::IsWindow(hwnd) && (::GetWindowLongPtr(hwnd, GWL_STYLE) & WS_CHILD)) {
      return gfx::Rect(0, TopAreaHeight(false), width(),
...
int BrowserFrameViewWin::TitlebarHeight(bool restored) const {
  ...
  if (::IsWindow(hwnd) && (::GetWindowLongPtr(hwnd, GWL_STYLE) & WS_CHILD) &&
      IsWebUITabStrip()) {
    return caption_button_container_->GetPreferredSize().height();
  }
  // end Add by TangramTeam
...
void BrowserFrameViewWin::PaintTitlebar(gfx::Canvas* canvas) const {
  ...
  // begin Add by TangramTeam
  SkColor titlebar_color = GetTitlebarColor();
  gfx::ScopedCanvas scoped_canvas(canvas);
  float scale = canvas->UndoDeviceScaleFactor();
  cc::PaintFlags flags;
  HWND hwnd = views::HWNDForWidget(frame());
  CommonUniverse::CBrowserImpl* browserImpl_ = nullptr;
  if (::IsWindow(hwnd) && (::GetWindowLongPtr(hwnd, GWL_STYLE) & WS_CHILD)) {
    if (g_pSpaceTelescopeImpl) {
      browserImpl_ = g_pSpaceTelescopeImpl->GetBrowserImpl(hwnd);
      if (browserImpl_ && browserImpl_->m_strBKColor == _T("")) {
        browserImpl_->m_strBKColor =
            browserImpl_->GetXobjAttr(_T("hostbrowserbkcolor"));
      }
    }
    if (browserImpl_ && browserImpl_->m_strBKColor != _T("")) {
      int a = 0, r = 0, g = 0, b = 0;
      _stscanf_s(browserImpl_->m_strBKColor, _T("ARGB(%d,%d,%d,%d)"), &a, &r,
                 &g, &b);
      titlebar_color = SkColorSetARGB(a, r, g, b);
