...
void KeyboardAccessTest::TestMenuKeyboardAccess(bool alternate_key_sequence,
  ...
  ASSERT_TRUE(
      ui_test_utils::NavigateToURL(browser(), GURL("tangram://version/")));
...
IN_PROC_BROWSER_TEST_F(KeyboardAccessTest, BackForwardKeys) {
  ...
  ASSERT_TRUE(
      ui_test_utils::NavigateToURL(browser(), GURL("tangram://version/")));
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("tangram://about/")));
