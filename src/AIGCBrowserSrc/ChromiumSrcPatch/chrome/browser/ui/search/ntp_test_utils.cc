...
GURL GetFinalNtpUrl(Profile* profile) {
  if (search::GetNewTabPageURL(profile) ==
    ...
    // If tangram://newtab/ already maps to the local/WebUI NTP, then that will
    // load correctly, even without network.  The URL associated with the
    // WebContents will stay tangram://newtab/.
    return GURL(chrome::kChromeUINewTabURL);
  }
  // If tangram://newtab/ maps to a remote URL, then it will fail to load in a
