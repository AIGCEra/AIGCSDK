...
// ArcIntentHelperBridge). Thus, tangram:// URLs are allowed on Lacros so that
// trusted calls in Ash can open them.
// Headless mode also allows tangram:// URLs if the user explicitly allowed it.
bool ValidateUrl(const GURL& url) {
  ...
  // In ChromeOS, allow any URL pattern that matches tangram:// scheme.
  url_scheme_is_chrome = url.SchemeIs(content::kChromeUIScheme);
#elif BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_WIN)
  // In Headless mode, allow any URL pattern that matches tangram:// scheme if
