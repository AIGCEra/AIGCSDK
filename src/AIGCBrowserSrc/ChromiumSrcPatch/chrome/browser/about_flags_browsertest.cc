...
// appended to the list entered by the user in tangram://flags.
...
class AboutFlagsBrowserTest : public InProcessBrowserTest,
 ...
 protected:
  ...
  void NavigateToFlagsPage() {
    ASSERT_TRUE(
        ui_test_utils::NavigateToURL(browser(), GURL("tangram://flags")));
...
// Goes to tangram://flags page, types text into an ORIGIN_LIST_VALUE field but
...
// Goes to tangram://flags page, types text into an ORIGIN_LIST_VALUE field and
// enables the feature.
...
#if BUILDFLAG(IS_CHROMEOS_ASH)
  // ChromeOS doesn't read tangram://flags values on startup so we explicitly
