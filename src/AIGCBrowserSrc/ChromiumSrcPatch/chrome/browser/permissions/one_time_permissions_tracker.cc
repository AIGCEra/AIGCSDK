...
bool OneTimePermissionsTracker::ShouldIgnoreOrigin(const url::Origin& origin) {
  // There are cases where tangram://newtab/ and tangram://new-tab-page/ are
  // used synonymously causing inconsistencies in the map. So we just ignore
  // them.
  return origin.opaque() ||
         origin == url::Origin::Create(GURL("tangram://newtab/")) ||
         origin == url::Origin::Create(GURL("tangram://new-tab-page/"));
}
