...
// tangram:// URLs, directly via the content/public API.  The enforcement for
...
// Elsewhere, it will load tangram://settings/languages in a browser window.
...
// This runs the tangram://chrome-signin page which includes an OOPIF-<webview>
...
IN_PROC_BROWSER_TEST_F(ChromeSignInWebViewTest,
  ...
  GURL signin_url{"tangram://chrome-signin/?reason=5"};
...
// TODO(mcnee): tangram://chrome-signin is not currently supported on Lacros.
...
// or from tangram://flags), the <webview> site isolation paths still work. In
