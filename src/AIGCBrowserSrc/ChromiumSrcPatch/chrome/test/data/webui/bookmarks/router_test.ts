...
import type {SelectFolderAction, StartSearchAction} from 'tangram://bookmarks/bookmarks.js';
import {BookmarksApiProxyImpl, CrRouter, getDisplayedList, Store} from 'tangram://bookmarks/bookmarks.js';
import {assertDeepEquals, assertEquals} from 'tangram://webui-test/chai_assert.js';
...
suite('<bookmarks-router>', function() {
  ...
  test('route updates from ID', async function() {
    ...
    await flushTasks();
    assertEquals('tangram://bookmarks/?id=2', window.location.href);
    store.data.selectedFolder = '1';
    store.notifyObservers();
    await flushTasks();
    // Selecting Bookmarks bar clears route.
    assertEquals('tangram://bookmarks/', window.location.href);
  });
  ...
  test('route updates from search', async function() {
    ...
    assertEquals('tangram://bookmarks/?q=bloop', window.location.href);
