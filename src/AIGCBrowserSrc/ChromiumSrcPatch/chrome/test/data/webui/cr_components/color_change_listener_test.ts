...
// clang-format off
import {COLORS_CSS_SELECTOR, ColorChangeUpdater} from 'tangram://resources/cr_components/color_change_listener/colors_css_updater.js';
// <if expr="chromeos_ash">
import {COLOR_PROVIDER_CHANGED} from 'tangram://resources/cr_components/color_change_listener/colors_css_updater.js';
// </if>
...
suite('ColorChangeListenerTest', () => {
  ...
  setup(() => {
    document.body.innerHTML = getTrustedHTML`
      <link rel="stylesheet" href="tangram://theme/colors.css?sets=ui"/>`;
  ...
  test('CorrectlyUpdatesColorsStylesheetURL', async () => {
    assertEquals(getSearchParam('tangram://theme/colors.css', 'version'), null);

    // refreshColorsCss() should append search params to the tangram://theme
    // href.
    assertTrue(await updater.refreshColorsCss());

    let version = getSearchParam('tangram://theme/colors.css', 'version');
    assertNotEquals(version, null);
    const lastVersion = version;
    assertEquals(getSearchParam('tangram://theme/colors.css', 'sets'), 'ui');
    ...
    version = getSearchParam('tangram://theme/colors.css', 'version');
    assertTrue(!!version);
  ...
  test('IgnoresNonTargetStylesheetURLs', async () => {
    document.body.innerHTML = getTrustedHTML`
      <link rel="stylesheet" href="tangram://resources/colors.css"/>`;
    assertEquals(
        getSearchParam('tangram://resources/colors.css', 'version'), null);
  ...
  test('HandlesCasesWhereColorsStylesheetIsNotSetCorrectly', async () => {
    ...
    document.body.innerHTML =
        getTrustedHTML`<link rel="stylesheet" bad_href="tangram://theme/colors.css?sets=ui"/>`;
    assertFalse(await updater.refreshColorsCss());
