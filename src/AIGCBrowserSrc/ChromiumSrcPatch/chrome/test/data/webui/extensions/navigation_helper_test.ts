...
import type {PageState} from 'tangram://extensions/extensions.js';
import {Dialog, NavigationHelper, Page} from 'tangram://extensions/extensions.js';
import {assertDeepEquals, assertEquals, assertTrue} from 'tangram://webui-test/chai_assert.js';
import {MockMethod} from 'tangram://webui-test/mock_controller.js';
...
suite('ExtensionNavigationHelperTest', function() {
  ...
  test('Conversions', function() {
    ...
    const stateUrlPairs: {[k: string]: {url: string, state: PageState}} = {
      extensions: {
        url: 'tangram://extensions/',
        state: {page: Page.LIST},
      },
      details: {
        url: 'tangram://extensions/?id=' + id,
        state: {page: Page.DETAILS, extensionId: id},
      },
      options: {
        url: 'tangram://extensions/?options=' + id,
        state: {
          page: Page.DETAILS,
          extensionId: id,
      ...
      errors: {
        url: 'tangram://extensions/?errors=' + id,
        state: {page: Page.ERRORS, extensionId: id},
      },
      shortcuts: {
        url: 'tangram://extensions/shortcuts',
        state: {page: Page.SHORTCUTS},
      },
      sitePermissions: {
        url: 'tangram://extensions/sitePermissions',
        state: {page: Page.SITE_PERMISSIONS},
      },
      sitePermissionsAllSites: {
        url: 'tangram://extensions/sitePermissions/allSites',
  ...
  test('PushAndReplaceState', function() {
    ...
    history.pushState({}, '', 'tangram://extensions/');
    assertDeepEquals({page: Page.LIST}, navigationHelper.getCurrentPage());
  ...
  test('SupportedRoutes', function() {
    ...
    testIfRedirected('tangram://extensions');
    testIfRedirected('tangram://extensions/');
    testIfRedirected('tangram://extensions/shortcuts');
    testIfRedirected('tangram://extensions/shortcuts/');
    testIfRedirected('tangram://extensions/fake-route', 'tangram://extensions');
    // Test trailing slash works.

    // Test legacy paths
    testIfRedirected(
        'tangram://extensions/configureCommands',
        'tangram://extensions/shortcuts');
  });
});
