...
import type {ActivityGroup, ActivityLogHistoryItemElement} from 'tangram://extensions/extensions.js';
...
suite('ExtensionsActivityLogHistoryItemTest', function() {
  ...
  test('count shown in descending order for multiple page URLs', function() {
    const countsByUrl =
        new Map([['google.com', 5], ['tangram://extensions', 10]]);
    ...
    assertTrue(pageUrls[0]!.querySelector<HTMLElement>('.page-url-link')!
                   .innerText!.includes('tangram://extensions'));
    assertEquals(
