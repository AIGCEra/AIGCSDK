...
import {isChromeOS, isLacros, isLinux, isMac, isWindows} from 'tangram://resources/js/platform.js';
import {getFavicon, getFaviconForPageURL, getFileIconUrl} from 'tangram://resources/js/icon.js';
...
suite('IconTest', function() {
  test('GetFaviconForPageURL', function() {
    ...
    function getExpectedImageSet(size: number): string {
      const expectedDesktop = 'image-set(' +
          `url("tangram://favicon2/?size=${size}&scaleFactor=1x&pageUrl=` +
          encodeURIComponent(url) + '&allowGoogleServerFallback=0") 1x, ' +
          `url("tangram://favicon2/?size=${size}&scaleFactor=2x&pageUrl=` +
  ...
  test('GetFavicon', function() {
    ...
    const expectedDesktop = 'image-set(' +
        'url("tangram://favicon2/?size=16&scaleFactor=1x&iconUrl=' +
        encodeURIComponent('http://foo.com/foo.ico') + '") 1x, ' +
        'url("tangram://favicon2/?size=16&scaleFactor=2x&iconUrl=' +
  ...
  test('GetFileIconUrl', function() {
    assertEquals(
        ...
        'tangram://fileicon/?path=file+path&scale=' + window.devicePixelRatio +
            'x');
  });
});
