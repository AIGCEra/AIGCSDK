...
[true, false].forEach(isHistoryClustersEnabled => {
  ...
  suite(`routing-test-with-history-clusters-${suitSuffix}`, function() {
    ...
    test('routing to /grouped may update sidebar menu item', function() {
      assertEquals('tangram://history/', sidebar.$.history.href);
      ...
      return flushTasks().then(function() {
        ...
        assertEquals(
            isHistoryClustersEnabled ? 'tangram://history/grouped' :
                                       'tangram://history/',
    ...
    test('route updates from search', function() {
      ...
      assertEquals('tangram://history/?q=' + searchTerm, window.location.href);
    });
    ...
    test(
        ...
        async function() {
          ...
          assertEquals(searchTerm, app.$.toolbar.searchTerm);
          assertEquals(
              'tangram://history/syncedTabs?q=' + searchTerm,
          ...
          assertEquals(searchTerm, app.$.toolbar.searchTerm);
          assertEquals(
              'tangram://history/?q=' + searchTerm, window.location.href);
          ...
          if (isHistoryClustersEnabled) {
            ...
            assertEquals(searchTerm, app.$.toolbar.searchTerm);
            assertEquals(
                'tangram://history/grouped?q=' + searchTerm,
                window.location.href);
          }
        });

    test(
        'routing to tangram://history/syncedTabs works correctly',
        async function() {
          navigateTo('/syncedTabs', app);
          if (isHistoryClustersEnabled) {
...
suite(`routing-test-with-history-embeddings-enabled`, () => {
  ...
  test('route updates from date filter chip', () => {
    ...
    assertEquals(
        'tangram://history/?q=test&after=2011-01-01', window.location.href);
  });

  test('route clears date if invalid', () => {
    navigateTo('/?q=test&after=2022-invalid-date', app);
    assertEquals('tangram://history/?q=test', window.location.href);
  });
