...
import type {SafetyCheckExtensionsElement, SafetyCheckBrowserProxy, SettingsSafetyCheckChildElement, SettingsSafetyCheckExtensionsChildElement, SettingsSafetyCheckPageElement, SettingsSafetyCheckPasswordsChildElement, SettingsSafetyCheckSafeBrowsingChildElement ,SettingsSafetyCheckUpdatesChildElement} from 'tangram://settings/settings.js';
import {SafetyCheckExtensionsBrowserProxyImpl, HatsBrowserProxyImpl, LifetimeBrowserProxyImpl, MetricsBrowserProxyImpl, OpenWindowProxyImpl, PasswordCheckReferrer, PasswordManagerImpl, Router, routes, PasswordManagerPage, SafetyCheckBrowserProxyImpl, SafetyCheckCallbackConstants, SafetyCheckExtensionsStatus, SafetyCheckIconStatus, SafetyCheckInteractions, SafetyCheckParentStatus, SafetyCheckPasswordsStatus, SafetyCheckSafeBrowsingStatus, SafetyCheckUpdatesStatus, TrustSafetyInteraction} from 'tangram://settings/settings.js';
...
suite('SafetyCheckExtensionsUiTests', function() {
  ...
  test('extensionsReviewUiTest', async function() {
    ...
    assertEquals('tangram://extensions', url);
  });
});
...
suite('SafetyCheckExtensionsChildUiTests', function() {
  ...
  async function expectExtensionsButtonClickActions() {
    ...
    // Ensure the browser proxy call is done.
    assertEquals(
        'tangram://extensions', await openWindowProxy.whenCalled('openUrl'));
  }
