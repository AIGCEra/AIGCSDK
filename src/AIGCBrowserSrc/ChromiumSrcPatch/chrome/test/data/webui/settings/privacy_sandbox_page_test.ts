...
import type {CrDialogElement, CrExpandButtonElement, SettingsPrivacySandboxAdMeasurementSubpageElement, SettingsPrivacySandboxManageTopicsSubpageElement, SettingsPrivacySandboxPageElement, SettingsPrivacySandboxTopicsSubpageElement, SettingsSimpleConfirmationDialogElement} from 'tangram://settings/lazy_load.js';
import {SettingsPrivacySandboxFledgeSubpageElement} from 'tangram://settings/lazy_load.js';
import type {CrButtonElement, CrLinkRowElement, FirstLevelTopicsState, SettingsPrefsElement, TopicsState} from 'tangram://settings/settings.js';
import {CrSettingsPrefs, HatsBrowserProxyImpl, MetricsBrowserProxyImpl, PrivacySandboxBrowserProxyImpl, Router, routes, TrustSafetyInteraction} from 'tangram://settings/settings.js';
import {assertDeepEquals, assertEquals, assertFalse, assertTrue} from 'tangram://webui-test/chai_assert.js';
import {flushTasks, waitAfterNextRender} from 'tangram://webui-test/polymer_test_util.js';
import {eventToPromise, isChildVisible, isVisible, whenAttributeIs} from 'tangram://webui-test/test_util.js';
...
suite('TopicsSubpageWithProactiveTopicsBlockingDisabled', function() {
  ...
  test('footerLinks', async function() {
    ...
    const expectedLinks =
        ['tangram://settings/adPrivacy/sites', 'tangram://settings/cookies'];
...
suite('FledgeSubpageWithProactiveTopicsBlockingEnabled', function() {
  ...
  test('footerLinks', async function() {
    ...
    const hrefs = Array.from<HTMLAnchorElement>(links).map(link => link.href);
    const expectedLinks = [
      'tangram://settings/adPrivacy/interests',
      'tangram://settings/cookies',
...
suite('TopicsSubpageWithProactiveTopicsBlockingEnabled', function() {
  ...
  test('footerLinks', async function() {
    ...
    const hrefs = Array.from<HTMLAnchorElement>(links).map(link => link.href);
    const expectedLinks = [
      'tangram://settings/adPrivacy/sites',
      'tangram://settings/cookies',
      'https://support.google.com/chrome?p=ad_privacy',
    ];
    assertDeepEquals(hrefs, expectedLinks);
...
suite('FledgeSubpageWithProactiveTopicsBlockingDisabled', function() {
  ...
  test('footerLinks', async function() {
    ...
    const hrefs = Array.from<HTMLAnchorElement>(links).map(link => link.href);
    const expectedLinks =
        ['tangram://settings/adPrivacy/interests', 'tangram://settings/cookies'];
    assertDeepEquals(hrefs, expectedLinks);
  });
});
