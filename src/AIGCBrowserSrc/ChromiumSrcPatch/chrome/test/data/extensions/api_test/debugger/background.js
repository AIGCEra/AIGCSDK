...
chrome.test.getConfig(config => chrome.test.runTests([
  ...
  async function attachToWebUI() {
    const tab = await openTab('tangram://version');
    const debuggee = {tabId: tab.id};
    chrome.debugger.attach(debuggee, protocolVersion,
        fail("Cannot access a tangram:// URL"));
    chrome.tabs.remove(tab.id);
  },
    ...
    chrome.debugger.attach(debuggee, protocolVersion, function() {
      ...
      function onDetach(from, reason) {
        ...
        chrome.debugger.attach(
            debuggee, protocolVersion, fail('Cannot access a tangram:// URL'));
      ...
      chrome.debugger.sendCommand(
        debuggee, "Page.navigate", {url: "tangram://version"}, onResponse);
    });
  },
