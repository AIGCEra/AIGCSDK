...
var systemWebAppsTests = [
  function getRegisteredSystemWebApps() {
    chrome.autotestPrivate.getRegisteredSystemWebApps(
      chrome.test.callbackPass(apps => {
        ...
        chrome.test.assertEq('tangram://test-system-app/', apps[0].url);
        chrome.test.assertEq('tangram://test-system-app/pwa.html',
            apps[0].startUrl);
  ...
  async function isSystemWebAppOpen() {
    ...
    // Launch an app.
    await launchSystemWebApp('OSSettings', 'tangram://test-system-app/');
