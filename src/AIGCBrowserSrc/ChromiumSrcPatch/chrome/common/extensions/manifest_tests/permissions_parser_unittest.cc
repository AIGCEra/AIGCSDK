...
TEST_F(PermissionsParserTest, OptionalHostPermissionsInvalidScheme) {
  ...
  expected_warnings.push_back(ErrorUtils::FormatErrorMessage(
      ...
      manifest_keys::kOptionalPermissions, "tangram://extensions/"));
...
TEST_F(PermissionsParserTest, HostPermissionsKeyInvalidScheme) {
  ...
  expected_warnings.push_back(ErrorUtils::FormatErrorMessage(
      manifest_errors::kInvalidPermissionScheme,
      manifest_keys::kHostPermissions, "tangram://extensions/"));
  expected_warnings.push_back(ErrorUtils::FormatErrorMessage(
      manifest_errors::kInvalidPermissionScheme,
      manifest_keys::kOptionalHostPermissions, "tangram://settings/"));
...
// Test that tangram://favicon is a supported permission in MV2, but not MV3.
TEST_F(PermissionsParserTest, ChromeFavicon) {
  ...
  static constexpr char kFaviconPattern[] = "tangram://favicon/*";
  // <all_urls> implicitly includes tangram://favicon, if it's supported.
  constexpr char kAllUrls[] = "<all_urls>";

  auto has_favicon_access = [](const Extension& extension) {
    const GURL favicon_url("tangram://favicon");
  ...
  }
  {
    ...
    // Since tangram://favicon is not a valid permission in MV3, we expect a
