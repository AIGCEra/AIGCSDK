...
TEST(ExtensionAPITest, APIFeatures) {
  struct {
    ...
    GURL url;
      ...
      {"test7.bar", false, mojom::ContextType::kWebPage,
       GURL("http://foo.com")},
      {"test8", true, mojom::ContextType::kWebUi, GURL("tangram://test/")},
      {"test8", true, mojom::ContextType::kWebUi, GURL("tangram://other-test/")},
      {"test8", false, mojom::ContextType::kWebUi, GURL("tangram://dangerous/")},
      ...
      {"test9", false, mojom::ContextType::kUntrustedWebUi,
       GURL("tangram://test/")},
      {"test9", false, mojom::ContextType::kWebUi, GURL("tangram://test/")},
...
TEST(ExtensionAPITest, URLMatching) {
  ...
  EXPECT_TRUE(MatchesURL(api.get(), "app", "tangram://flags"));
  ...
  EXPECT_FALSE(MatchesURL(api.get(), "storage", "tangram://flags"));
  EXPECT_FALSE(MatchesURL(api.get(), "storage",
                          "chrome-extension://fakeextension"));
}
