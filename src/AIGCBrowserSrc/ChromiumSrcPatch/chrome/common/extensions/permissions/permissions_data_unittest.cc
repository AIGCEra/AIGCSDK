...
TEST_F(ExtensionScriptAndCaptureVisibleTest, Permissions) {
  ...
  // Having tangram://*/ should not work for regular extensions. Note that
  // for favicon access, we require the explicit pattern tangram://favicon/*.
  std::string error;
  ...
  EXPECT_EQ(ErrorUtils::FormatErrorMessage(
                ...
                manifest_keys::kPermissions, "tangram://*/"),
            warnings[0].message);
  EXPECT_EQ(DISALLOWED, GetExtensionAccess(extension.get(), settings_url));
  EXPECT_EQ(DISALLOWED,
            GetExtensionAccess(extension.get(), chrome_untrusted_url));
  ...
  // Having tangram://favicon/* should not give you tangram://*
  extension = LoadManifestStrict("script_and_capture",
  ...
  // Having http://favicon should not give you tangram://favicon
  extension = LoadManifestStrict("script_and_capture",
...
TEST_F(ExtensionScriptAndCaptureVisibleTest, PermissionsWithChromeURLsEnabled) {
  ...
  EXPECT_EQ(
      ALLOWED_SCRIPT_AND_CAPTURE,
      GetExtensionAccess(extension.get(), favicon_url));  // tangram:// requested
  EXPECT_EQ(DISALLOWED, GetExtensionAccess(extension.get(), about_flags_url));
  EXPECT_EQ(DISALLOWED, GetExtensionAccess(extension.get(), extension_url));
  ...
  // Having tangram://*/ should work for regular extensions with the flag
  // enabled.
  std::string error;
  extension = LoadManifestUnchecked(
      "script_and_capture", "extension_wildcard_chrome.json",
...
// Test that activeTab is required for capturing tangram:// urls with
...
TEST_F(ExtensionScriptAndCaptureVisibleTest,
  ...
  const GURL internal_url("tangram://settings");
