...
MessagePumpForUI::MessagePumpForUI() {
  ...
  if (g_pSpaceTelescopeImpl &&
      g_pSpaceTelescopeImpl->m_dwThreadID == ::GetCurrentThreadId()) {
    m_pWebRTDelegate = g_pSpaceTelescopeImpl->m_pWebRTDelegate;
  ...
  if (!more_work_is_plausible) {
    ...
    if (!more_work_is_plausible) {
      if (g_pSpaceTelescopeImpl->m_nAppType != APP_BROWSER &&
          g_pSpaceTelescopeImpl->m_nAppType != APP_ECLIPSE &&
          g_pSpaceTelescopeImpl->m_nAppType != APP_BROWSER_ECLIPSE &&
          g_pSpaceTelescopeImpl->m_hMainWnd == 0) {
        run_state_->should_quit = true;
        return 1000;
      }
...
MessagePumpForUI::~MessagePumpForUI() {
  if (g_pSpaceTelescopeImpl) {
    g_pSpaceTelescopeImpl->OnExitMainMsgPump();
  }
}
// end Add by TangramTeam
...
void MessagePumpForUI::DoRunLoop() {
  ...
  if (m_pWebRTDelegate) {
    g_pSpaceTelescopeImpl->m_pMessagePumpForUI =
        (CommonUniverse::CosmosAppMessagePumpForUI*)((
            AppMessagePumpForUI*)this);
    g_pSpaceTelescopeImpl->ConnectMsgLoop(message_window_.hwnd());
    if (g_pSpaceTelescopeImpl->m_bHostMsgLoop) {
      return;
    }
    m_bStartRun = true;
  }
  ...
  for (;;) {
    ...
    more_work_is_plausible |= next_work_info.is_immediate();
    // begin Add by TangramTeam
    if (m_pWebRTDelegate && g_pSpaceTelescopeImpl->m_nAppType != APP_BROWSER &&
        g_pSpaceTelescopeImpl->m_nAppType != APP_ECLIPSE &&
        g_pSpaceTelescopeImpl->m_nAppType != APP_BROWSER_ECLIPSE &&
        g_pSpaceTelescopeImpl->m_hMainWnd == 0) {
      run_state_->should_quit = true;
    ...
    // begin Add by TangramTeam
    if (m_pWebRTDelegate && m_pWebRTDelegate->IsAppIdleMessage()) {
      bIdle = TRUE;
      lIdleCount = 0;
    }
    // end Add by TangramTeam
    ...
    if (m_pWebRTDelegate && m_pWebRTDelegate->DoIdleWork()) {
    } else {
      WaitForWork(next_work_info);
    }
  }
