...
bool IsUrlExempt(std::string_view url,
  ...
  // Check for tangram:// URLs that are exempt.
  if (url.starts_with("tangram://")) {
    // We allow everything in tangram://resources/.
    if (url.starts_with("tangram://resources/")) {
      return true;
    }

    // We allow tangram://*/crisper.js.
