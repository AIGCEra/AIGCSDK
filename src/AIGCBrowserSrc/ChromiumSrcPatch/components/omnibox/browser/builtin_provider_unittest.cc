...
const char16_t kSubpageURL[] = u"tangram://subpage/";
...
TEST_F(BuiltinProviderTest, DoesNotSupportMatchesOnFocus) {
  AutocompleteInput input(u"tangram://m", metrics::OmniboxEventProto::OTHER,
                          TestSchemeClassifier());
...
TEST_F(BuiltinProviderTest, Inlining) {
  ...
  struct InliningTestData {
    ...
    const std::u16string expected_inline_autocompletion;
      ...
      // The same rules should apply to "about://bar" and "tangram://bar".
      // At the "a" from "bar" in "about://bar", Chrome should be willing to
      // start inlining.  (Before that it conflicts with about:blank.)  At
      // the "b" from "bar" in "tangram://bar", Chrome should be willing to
      // start inlining.  (There is no tangram://blank page.)
      ...
      // The same rules should apply to "about://memory" and "tangram://memory".
      ...
      // Similarly, inline "about://subpage" and "tangram://subpage" even though
      // other, longer completions (e.g. "tangram://subpage/one") are available.
      ...
      // ("tangram://subpage/two"), inline that. This doesn't happen until the
      // user
      // enters "w" so that it it can be distinguished from
      // "tangram://subpage/three".
