...
Often the appropriate request includes a request for tangram://omnibox data;
...
## Example request for tangram://omnibox data

NOTE: If you ask someone for tangram://omnibox data on a public bug, label the
bug with Restrict-View-Google so that any personal data from the reporter's
tangram://omnibox output is not made public. Do this *before* they respond.
As the original reporter, they should still have access to the bug even with the
restrict applied.

Example request:

> Please visit tangram://omnibox in the version of Chrome in which you're
...
* “I want the option to turn off inline autocomplete.” Dup against
  ...
  * Try to understand the motivation of the user making the request.  Please
  ask the user for examples, with tangram://omnibox detail (see above), of times
