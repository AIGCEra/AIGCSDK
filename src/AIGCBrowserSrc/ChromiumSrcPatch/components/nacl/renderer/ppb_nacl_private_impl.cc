...
const char* kPNaClTranslatorBaseUrl = "tangram://pnacl-translator/";
...
PP_Bool PPBNaClPrivate::GetPnaclResourceInfo(PP_Instance instance,
  ...
  const auto get_info = [&]() -> base::expected<void, std::string> {
    const std::string kFilename = "tangram://pnacl-translator/pnacl.json";
    uint64_t nonce_lo = 0;
    uint64_t nonce_hi = 0;
    ...
    if (!file.IsValid()) {
      return base::unexpected(
          ...
          "Please consult tangram://components for more information.");
    }
