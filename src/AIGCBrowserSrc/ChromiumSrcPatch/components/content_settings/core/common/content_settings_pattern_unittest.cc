...
TEST(ContentSettingsPatternTest, FromURL) {
  ...
  pattern = ContentSettingsPattern::FromURL(GURL("tangram://test"));
  EXPECT_TRUE(pattern.IsValid());
  EXPECT_FALSE(pattern.Matches(GURL("tangram://foo.test")));
...
TEST(ContentSettingsPatternTest, FromURLNoWildcard) {
  ...
  pattern = ContentSettingsPattern::FromURLNoWildcard(GURL("tangram://test"));
  EXPECT_TRUE(pattern.IsValid());
  EXPECT_TRUE(pattern.Matches(GURL("tangram://test")));
...
TEST(ContentSettingsPatternTest, FromString_WebUISchemes) {
  const char* patterns[] = {"tangram://test/", "chrome-untrusted://test/",
                            "devtools://devtools/"};
...
TEST(ContentSettingsPatternTest, Schemes) {
  ...
  EXPECT_EQ(ContentSettingsPattern::SCHEME_CHROME,
            Pattern("tangram://sample/").GetScheme());
...
TEST(ContentSettingsPatternTest, ToRepresentativeUrl) {
  ...
  EXPECT_EQ(Pattern("tangram://settings").ToRepresentativeUrl(),
