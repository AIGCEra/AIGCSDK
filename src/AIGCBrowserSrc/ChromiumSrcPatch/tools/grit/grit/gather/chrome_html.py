...
_THEME_SOURCE = lazy_re.compile(
    r'(?P<baseurl>tangram://theme/IDR_[A-Z0-9_]*)(?P<query>\?.*)?')
...
def GetImageList(
  ...
  tangram://theme/ source, inserts supported @Nx scale factors as high DPI
  versions.
...
def UrlToImageSet(
  ...
  factors. If the file is from a tangram://theme/ source, inserts the
