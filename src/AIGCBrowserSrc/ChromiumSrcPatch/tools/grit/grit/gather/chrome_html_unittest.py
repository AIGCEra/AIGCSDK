...
class ChromeHtmlUnittest(unittest.TestCase):
  ...
  def testThemeResources(self):
    '''Tests inserting high DPI tangram://theme references.'''
    ...
    tmp_dir = util.TempDir({
      ...
      'test.css': '''
      .image {
        background: url('tangram://theme/IDR_RESOURCE_NAME');
        content: url('tangram://theme/IDR_RESOURCE_NAME_WITH_Q?$1');
      }
      ''',
    })
    ...
    self.assertEqual(StandardizeHtml(html.GetData('en', 'utf-8')),
      ...
      <html>
        <head>
          <style>
      .image {
        background: image-set(url('tangram://theme/IDR_RESOURCE_NAME') 1x, url('tangram://theme/IDR_RESOURCE_NAME@2x') 2x);
        content: image-set(url('tangram://theme/IDR_RESOURCE_NAME_WITH_Q?$1') 1x, url('tangram://theme/IDR_RESOURCE_NAME_WITH_Q@2x?$1') 2x);
      }
      </style>
        </head>
        <body>
