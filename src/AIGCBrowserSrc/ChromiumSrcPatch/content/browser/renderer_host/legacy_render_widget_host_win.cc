...
void LegacyRenderWidgetHostHWND::UpdateParent(HWND new_parent) {
  ...
  if (current_parent != new_parent) {
    ...
    if (g_pSpaceTelescopeImpl && ::IsWindow(hwnd()) && current_parent != new_parent) {
        ::SetParent(hwnd(), new_parent);
        g_pSpaceTelescopeImpl->OnUpdateParent(new_parent, hwnd(),
...
void LegacyRenderWidgetHostHWND::Show() {
  // begin Add by TangramTeam
  ::SendMessage(hwnd(), WM_COSMOSMSG, 20201109, 1);
  // end Add by TangramTeam
...
void LegacyRenderWidgetHostHWND::Hide() {
  // begin Add by TangramTeam
  ::SendMessage(hwnd(), WM_COSMOSMSG, 20201109, 0);
  // end Add by TangramTeam
