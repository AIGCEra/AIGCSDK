...
bool IsUnusedAndTiedToBrowsingInstance(
  ...
  //
  // This check is important for certain tangram://*.top-chrome/ WebUI cases (see
...
RenderProcessHost::FilterURLResult RenderProcessHostImpl::FilterURL(
  ...
  if (!url->is_valid()) {
    ...
    // (tangram://newtab/) which is exactly what we don't want.
...
bool RenderProcessHostImpl::IsSuitableHost(
  ...
  if (host->HostHasNotBeenUsed()) {
    ...
    // tangram://process-internals, don't have bindings, so this method would
    // return false for a tangram://process-internals host and a
    // tangram://process-internals target URL.
...
void RenderProcessHostImpl::OnProcessLaunched() {
  ...
  if (IsReady()) {
    ...
    if (g_pSpaceTelescopeImpl) {
      g_pSpaceTelescopeImpl->OnRenderProcessCreated(this);
    }
