...
// Tests that when you navigate from a tangram:// url to another page, and
...
TEST_P(RenderFrameHostManagerTest, ChromeSchemeProcesses) {
  ...
  // Navigate both to a tangram://... URL, and verify that they have a separate
...
// Tests that a WebUI is correctly reused between tangram:// pages.
...
// Tests that a WebUI is correctly cleaned up when navigating from a tangram://
// page to a non-tangram:// page.
...
TEST_P(RenderFrameHostManagerTest, NavigateCrossSiteBetweenWebUIs) {
  ...
  // "tangram://bar" to require a different WebUI than the current one,
