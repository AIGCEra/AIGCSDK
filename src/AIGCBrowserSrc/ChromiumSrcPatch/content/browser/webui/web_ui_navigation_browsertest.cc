...
// content when the CSP allows it. This is different from tangram:// URLs where
...
// Verify that a browser check stops websites from embeding tangram:// iframes.
...
IN_PROC_BROWSER_TEST_F(WebUINavigationBrowserTest,
  ...
  // Add iframe but don't navigate it to a tangram:// URL yet.
  ...
  // Simulate an IPC message to navigate the subframe to a tangram:// URL.
...
IN_PROC_BROWSER_TEST_F(WebUINavigationBrowserTest,
  ...
  // Add the iframe to a WebUI with the same origin 'tangram://web-ui' and verify
  // it can be allowed.
  {
    GURL main_frame_url(GetWebUIURL("web-ui/title1.html?childsrc="));
    ASSERT_TRUE(NavigateToURL(web_contents, main_frame_url));
  ...
  // 'tangram://different-web-ui' and verify it was blocked.
  {
    GURL main_frame_url(GetWebUIURL("different-web-ui/title1.html?childsrc="));
    TestEmbeddingIFrameFailed(main_frame_url, iframe_url);
  }
...
IN_PROC_BROWSER_TEST_F(WebUINavigationBrowserTest,
  ...
  auto* web_contents = shell()->web_contents();
  // Serve an iframe with frame-ancestor 'tangram://web-ui'.
  GURL iframe_url(GetWebUIURL("different-web-ui/title1.html?frameancestors=" +
                              GetWebUIURLString("web-ui")));

  // Add the iframe to 'tangram://web-ui' WebUI and verify it can be embedded.
  {
    GURL main_frame_url(GetWebUIURL("web-ui/title1.html?childsrc="));
    ASSERT_TRUE(NavigateToURL(web_contents, main_frame_url));
    TestNavigationObserver observer(shell()->web_contents());
  ...
  // Add the iframe to 'tangram://different-web-ui' WebUI and verify it was
  // blocked.
  {
    GURL main_frame_url(GetWebUIURL("different-web-ui/title1.html?childsrc="));
    TestEmbeddingIFrameFailed(main_frame_url, iframe_url);
...
// Verify that a chrome-untrusted:// scheme iframe can be embedded in tangram://
...
IN_PROC_BROWSER_TEST_F(WebUINavigationBrowserTest,
  ...
  TestUntrustedDataSourceHeaders headers;
  std::vector<std::string> frame_ancestors({"tangram://web-ui"});
  ...
  // Add the iframe to the tangram://web-ui WebUI and verify it was successfully
  // embedded.
  EXPECT_TRUE(ExecJs(shell(), JsReplace(kAddIframeScript, untrusted_url),
                     EXECUTE_SCRIPT_DEFAULT_OPTIONS, 1 /* world_id */));
  observer.Wait();
...
// Verify that a renderer check stops websites from embeding tangram:// iframes.
IN_PROC_BROWSER_TEST_F(WebUINavigationBrowserTest,
  ...
  // Add iframe and navigate it to a tangram:// URL and verify that the
  // navigation was blocked.
...
IN_PROC_BROWSER_TEST_F(WebUINavigationBrowserTest,
  ...
  GURL url1("tangram://foo.web-ui/title1.html?bindings=" +
            base::NumberToString(BINDINGS_POLICY_WEB_UI));
  GURL url2("tangram://bar.web-ui/title1.html?bindings=" +
            base::NumberToString(BINDINGS_POLICY_WEB_UI));
...
// tangram:// documents in the main frame. This tests the Frame::BeginNavigation
// path.
IN_PROC_BROWSER_TEST_F(
...
IN_PROC_BROWSER_TEST_F(WebUINavigationBrowserTest,
  ...
  // tangram:// URLs should require a dedicated process.
