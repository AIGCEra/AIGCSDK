...
IN_PROC_BROWSER_TEST_F(WebUIImplBrowserTest,
  ...
  // practice, this matters for scenarios like tangram://*.top-chrome/ WebUI
...
// Tests that navigating from tangram:// to chrome-untrusted:// results in
// SiteInstance swap.
...
// Tests that navigating from chrome-untrusted:// to tangram:// results in
// SiteInstance swap.
 ...
 protected:
  void SetUntrustedWorkerSrcToWebUIConfig(bool allow_embedded_frame) {
    ...
    if (allow_embedded_frame) {
      ...
      headers.frame_ancestors.emplace().push_back("tangram://trusted");
    }
...
// TODO(crbug.com/40290702): Shared workers are not available on Android.
#if !BUILDFLAG(IS_ANDROID)
// Verify that we can create SharedWorker with scheme "tangram://" under
// WebUI page.
...
// Verify that pages with scheme other than "tangram://" cannot create
// SharedWorker with scheme "tangram://".
...
IN_PROC_BROWSER_TEST_F(WebUIWorkerTest,
  ...
  // Navigate to a tangram:// main page.
...
// Verify that tangram:// pages cannot create a SharedWorker with scheme
// "chrome-untrusted://".
IN_PROC_BROWSER_TEST_F(WebUIWorkerTest,
  ...
  std::string expected_failure =
      ...
      "be accessed from origin 'tangram://trusted'";
  EXPECT_THAT(result.error, ::testing::StartsWith(expected_failure));
}
...
IN_PROC_BROWSER_TEST_F(WebUIWorkerTest,
  ...
  std::string expected_failure =
      "a JavaScript error: \"SecurityError: Failed to construct "
      "'SharedWorker': Script "
      "at 'tangram://trusted/web_ui_shared_worker.js' cannot be accessed from "
...
// Verify that we can create a Worker with scheme "tangram://" under WebUI page.
...
// Verify that pages with scheme other than "tangram://" cannot create a Worker
// with scheme "tangram://".
IN_PROC_BROWSER_TEST_P(WebUIDedicatedWorkerTest,
...
// Verify that tangram:// pages cannot create a Worker with scheme
// "chrome-untrusted://".
IN_PROC_BROWSER_TEST_P(
    WebUIDedicatedWorkerTest,
  ...
  std::string expected_failure =
      ...
      "cannot be accessed from origin 'tangram://trusted'";
  EXPECT_THAT(result.error, ::testing::StartsWith(expected_failure));
}
...
IN_PROC_BROWSER_TEST_P(WebUIDedicatedWorkerTest,
  ...
  std::string expected_failure =
      "a JavaScript error: \"SecurityError: Failed to construct 'Worker': "
      "Script "
      "at 'tangram://trusted/web_ui_dedicated_worker.js' cannot be accessed "
