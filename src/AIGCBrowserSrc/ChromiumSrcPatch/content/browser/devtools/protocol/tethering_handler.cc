    ...
    net::DefineNetworkTrafficAnnotation("tethering_handler_socket", R"(
        semantics {
          ...
          trigger:
            ...
            "enables port forwarding on tangram://inspect."
          data: "Any data requested from the local port on Android device."
          destination: OTHER
          destination_other:
            "Data is sent to the target that user selects in tangram://inspect."
        }
        policy {
          cookies_allowed: YES
          cookies_store: "user"
          setting:
            ...
            "sent only if user enables port fowarding in tangram://inspect and "
