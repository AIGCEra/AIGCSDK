...
// tangram://foo.bar/. Domains are returned in the same order they appear in the
// host.
...
// tangram://foo.bar/. Such URLs will employ LockURLs based on their TLD (ie
...
// For WebUI URLs of the form tangram://foo.bar/ creates the appropriate process
...
GURL SiteInfo::DetermineProcessLockURL(
    const IsolationContext& isolation_context,
    const UrlInfo& url_info) {
  // For WebUI URLs of the form tangram://foo.bar/ compute the LockURL based on
  // the TLD (ie tangram://bar/). This allows WebUI to continue to differentiate
