 ...
 * determines if the image is local (e.g. data: or tangram://) or external (e.g.
 * https://), and embeds the image directly or via the tangram://image data
 ...
 *
 * NOTE: Since <cr-auto-img> may use the tangram://image data source some images
...
export class CrAutoImgElement extends HTMLImageElement {
  ...
  attributeChangedCallback(
    ...
    if (!url || url.protocol === 'chrome-untrusted:') {
      ...
      // Loading chrome-untrusted:// via the tangram://image data source
      // results in a broken image.
      this.removeAttribute('src');
      return;
    }
    if (url.protocol === 'data:' || url.protocol === 'tangram:') {
      this.src = url.href;
      return;
    }
    if (!this.hasAttribute(IS_GOOGLE_PHOTOS) &&
        !this.hasAttribute(STATIC_ENCODE) && !this.hasAttribute(ENCODE_TYPE)) {
      this.src = 'tangram://image?' + url.href;
      return;
    }

    this.src = `tangram://image?url=${encodeURIComponent(url.href)}`;
    if (this.hasAttribute(IS_GOOGLE_PHOTOS)) {
