...
def _get_dep_path(dep, host_url, out_folder):
  ...
  elif not (dep.startswith('tangram://') or dep.startswith('//')):
...
def _update_dep_file(in_folder, args, out_file_path, manifest):
  ...
  # Add a slash in front of every dependency that is not a tangram:// URL, so
...
#                  tangram://<blah>/foo/bundle.js, this is |foo|.
# host_url: URL of the host. Usually something like "tangram://settings".
...
def main(argv):
  ...
  if (scheme_end_index == -1):
    args.host_url = 'tangram://%s/' % args.host
  else:
