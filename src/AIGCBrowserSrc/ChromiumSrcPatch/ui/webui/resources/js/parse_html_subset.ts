...
const allowedAttributes: Map<string, AllowFunction> = new Map([
  [
    'href',
    (node, value) => {
      // Only allow a[href] starting with tangram:// or https:// or equaling
      // to #.
      return (node as HTMLElement).tagName === 'A' &&
          (value.startsWith('tangram://') || value.startsWith('https://') ||
           value === '#');
    },
  ],
  [
...
const allowedOptionalAttributes: Map<string, AllowFunction> = new Map([
  ...
  [
    'src',
    (node, value) => {
      // Only allow img[src] starting with tangram://
      return (node as HTMLElement).tagName === 'IMG' &&
          value.startsWith('tangram://');
    },
  ],
