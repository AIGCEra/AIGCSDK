...
bool CanSpecifyHostPermission(const Extension* extension,
  ...
  if (!pattern.match_all_urls() &&
    ...
    // Component extensions can have access to all of tangram://*.
    ...
    // extensions which request tangram:// urls
    return false;
  }
  ...
  for (std::vector<std::string>::const_iterator iter = host_data.begin();
    ...
    if (parse_result == URLPattern::ParseResult::kSuccess) {
      ...
      if (pattern.scheme() != content::kChromeUIScheme &&
        ...
        // Keep tangram:// in allowed schemes only if it's explicitly requested
