...
// These patterns would otherwise match hosts like tangram://settings, which
...
std::unique_ptr<PermissionSet> AdjustHostPatterns(
  ...
  // If the extension is allowed to run on tangram:// URLs, then we don't have
  // to adjust anything.
  ...
  // allow for patterns like tangram://favicon).
...
void PermissionsManager::OnUserPermissionsSettingsChanged() {
  ...
  // entries in the tangram://extensions page for disabled extensions are
  // accurate.
