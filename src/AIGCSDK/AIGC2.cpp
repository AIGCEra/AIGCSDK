...
namespace CommonUniverse {
	...
	CAIGCWinAppDelegate* g_pWebRTDelegate = nullptr;
	...
	CString ComputeHash(CString source)
	{
		...
		CString strRet = _T("");
		CalculateByteMD5((BYTE*)strSrc.c_str(), strSrc.length(), strRet);
		return strRet;
	}
	...
	static LRESULT WebRTCBTProc(int nCode, WPARAM wParam, LPARAM lParam)
	{
		...
		if (nCode == HCBT_CREATEWND) {
			...
			if (g_pWebRTDelegate && g_pWebRTDelegate->IsBrowserModel(false)) {
				//Afx:00007FF6F5B50000:8:0000000000010003:0000000000000000:000000000082078D
				if (strClassName.Find(_T("Afx:") == 0)) {
			...
			//				pAppProxy = static_cast<IUniverseAppProxy*>((CAIGCWinAppDelegate*)pApp);
			//			}
			//			else {
	...
	CAIGCWinAppDelegate::CAIGCWinAppDelegate() {
		m_strProviderID = _T("");
		m_nShellCmd = CCommandLineInfo::FileNew;
		m_pvoid = (void*)1;
		InitApplication();
		g_pWebRTDelegate = this;
	}
	...
	BOOL CAIGCWinAppDelegate::InitApplication() {
		if (WebRTInit(_T("")))
	...
	HWND CAIGCWinAppDelegate::GetActivePopupMenu(HWND hWnd) {
	...
	CString CAIGCWinAppDelegate::GetNames() {
	...
	CString CAIGCWinAppDelegate::GetTags(CString strName) {
	...
	HWND CAIGCWinAppDelegate::Create(HWND hParentWnd, IXobj* pXobj) {
	...
	bool CAIGCWinAppDelegate::WebRTInit(CString strID) {
		if (!afxContextIsDLL) {
		...
		if (hUniverseModule) {
			...
			if (!afxContextIsDLL) {
				...
				g_pSpaceTelescopeImpl->m_pWebRTDelegate = (IWebRTDelegate*)this;
	...
	CString CAIGCWinAppDelegate::QueryDocType(HWND hWnd) {
	...
	CString CAIGCWinAppDelegate::QueryWndClassName(HWND hWnd) {
	...
	HWND CAIGCWinAppDelegate::GetFrameWnd(HWND hWnd, int& nType) {
		CWnd* pWnd = CWnd::FromHandle(hWnd);
		if (pWnd) {
			CFrameWnd* pFrame = pWnd->GetParentFrame();
			if (pFrame) {
	...
	void CAIGCWinAppDelegate::AttachCDockablePane(CDockablePane* pDockablePane, WebRTFrameWndInfo* pWebRTFrameWndInfo)
	{
	...
	HWND CAIGCWinAppDelegate::QueryWndInfo(QueryType nType, HWND hWnd) {
		...
		if (hWnd == NULL && m_pMainWnd) {
			...
			if (m_pDockingManager == NULL) {
				...
				m_nShellCmd = cmdInfo.m_nShellCommand;
				if (AfxGetApp()->m_pMainWnd->IsKindOf(RUNTIME_CLASS(CFrameWnd))) {
					WebRTFrameWndInfo* pWebRTFrameWndInfo = nullptr;
					...
					m_pDockingManager = m_GlobalUtils.GetDockingManager(m_pMainWnd);
					if (m_pDockingManager) {
						if (AfxGetApp()->m_pMainWnd->IsKindOf(RUNTIME_CLASS(CMDIFrameWnd))) {
					...
					if (pWebRTFrameWndInfo) {
						if (m_pMainWnd->IsKindOf(RUNTIME_CLASS(CFrameWnd)))
						{
							...
							while (nPos) {
								CDocTemplate* pTemplate = AfxGetApp()->GetNextDocTemplate(nPos);
	...
	bool CAIGCWinAppDelegate::SetFrameCaption(HWND hWnd,
		CString strCaption,
		CString strAppName) {
	...
	CString CAIGCWinAppDelegate::QueryParentInfo(HWND hPWnd, void* lpInfo) {
	...
	void CAIGCWinAppDelegate::OpenDocFile(CString strFileName,
		CString strExt,
	...
	BOOL CAIGCWinAppDelegate::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
	{
	...
	void CAIGCWinAppDelegate::OnIPCMsg(CWebViewImpl* pWebViewImpl,
		CString strType,
		CString strParam1,
		CString strParam2,
		CString strParam3,
	...
	CXobjProxy* CAIGCWinAppDelegate::OnXobjInit(IXobj* pNewNode) {
		CComBSTR bstrName("");
	...
	CNucleusProxy* CAIGCWinAppDelegate::OnNucleusCreated(INucleus* pNewFrame) {
		__int64 h = 0;
	...
	CNucleiProxy* CAIGCWinAppDelegate::OnNucleiCreated(INuclei* pNuclei) {
	...
	void CAIGCWinAppDelegate::GetWebRTInfo() {
		...
		{
			g_strChromeRTFilePath = CString(_T(""));
			CString strData = _T("");
	...
	CString CAIGCWinAppDelegate::GetDocTemplateID(CDocument* pDoc) {
		if (pDoc) {
	...
	bool CAIGCWinAppDelegate::AttachWebRT() {
		HMODULE hModule = ::GetModuleHandle(_T("universe.dll"));
		if (hModule) {
			GetWebRTImpl _pWebRTImplFunction;
			_pWebRTImplFunction = (GetWebRTImpl)GetProcAddress(hModule, "GetWebRTImpl");
	...
	bool CAIGCWinAppDelegate::ProcessAppType(bool bCrashReporting) {
		BOOL bWin32 =
			(::PathFileExists(g_pSpaceTelescopeImpl->m_strExeName + _T(".app.html")) == false) &&
			(::PathFileExists(g_pSpaceTelescopeImpl->m_strExeName + _T(".html")) == false);
		if (bWin32)
			return true;
		if (g_pSpaceTelescopeImpl) {
	...
	BOOL CAIGCWinAppDelegate::IsBrowserModel(bool bCrashReporting) {
		BOOL bWin32 =
			(::PathFileExists(CString(AfxGetApp()->m_pszExeName) + _T(".app.html")) == false) &&
			(::PathFileExists(CString(AfxGetApp()->m_pszExeName) + _T(".html")) == false);
		if (bWin32 == false) {
			HMODULE hModule = ::GetModuleHandle(L"AIGCAgent.dll");
			if (hModule == nullptr)
	...
	HWND CAIGCWinAppDelegate::GetDockablePane(HWND hFrame, int nID) {
	...
	RECT CAIGCWinAppDelegate::GetClientAreaBounds()
	{
		if (m_pDockingManager)
	...
	void CAIGCWinAppDelegate::CalcWindowRectForMDITabbedGroups(LPRECT rc)
	{
		if (m_pMDIClientAreaWnd)
			((CMDIClientAreaWnd*)m_pMDIClientAreaWnd)->CalcWindowRectForMDITabbedGroups(rc, 0);
	}

	//CAIGCWinAppDelegate theAppProxy;
